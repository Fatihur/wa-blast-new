...
const App: React.FC = () => {
  ...
  const [apiSettings, setApiSettings] = useLocalStorage<ApiSettings>('apiSettings', { provider: 'fonnte', fonnteApiKey: '', geminiApiKey: '', baileysSessionStatus: 'disconnected', antiBan: { delay: 4, quota: 100 } });
  ...
  const renderView = () => {
    ...
    switch (mainView) {
      case 'dashboard':
        return <Dashboard campaigns={campaigns} contacts={contacts} />;
      case 'blast':
  ...
  const isApiConfigured = useMemo(() => {
      ...
      const isWaProviderSet = (apiSettings.provider === 'fonnte' && !!apiSettings.fonnteApiKey) ||
